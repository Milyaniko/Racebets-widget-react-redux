...
- Clicking on the filter button displays a new data and removes the previous one. All the data sorted by category and displays from Top    to Down by the highest value;
...
- Because of the local JSON file, we don't need to use some specific tools to catch the data. It's only necessary to import. While in case with a real external API it's better to use Async requests with some tool, personally I prefer Axios because of the format and supportance of all the main browsers.

- In case of external API it's also possible to display the the data not separately by each other, but by comparison between their values, for example ( aCategoryValue > bCategoryValue ? aCategory : bCategory ). This functionality is optional and could be implemented by necessarity.
